...
def _event_metadata(trajectories, action_horizon):
    ...
    for traj in trajectories:
        ...
        n_chunks = len(traj["actions"]) - action_horizon
        if n_chunks <= 0:
            continue
...
def evaluate_run(run_dir, split, checkpoint_name, batch_size, device):
    ...
    current = metadata["current_labels"]
    rises = metadata["rise_in_horizon"]

    if len(probs) != len(targets) or len(probs) != len(current):
    ...
    for idx, name in enumerate(config["label_names"]):
        ...
        masks = {
            "current_false_rise_in_horizon": (~current[:, idx]) & rises[:, idx],
            "current_false_target_positive_no_rise": (~current[:, idx]) & (targets[:, idx] > 0.5) & (~rises[:, idx]),
            "current_false_target_negative": (~current[:, idx]) & (targets[:, idx] <= 0.5),
            "current_true": current[:, idx],
        }
        groups = {}
...
def print_report(report):
    ...
    for name, result in report["label_results"].items():
        ...
        print(
            f"  {name:<16} pos={result['target_positive_rate']:.3f} "
