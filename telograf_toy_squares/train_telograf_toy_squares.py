#!/usr/bin/env python
"""Train a TeLoGraF-style Toy Squares low-dimensional flow policy."""
...
class LocalTemporalFlowNet(nn.Module):
    """Small flow-matching fallback for environments missing TeLoGraF deps."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data", type=Path, required=True, help="Generated Toy Squares TeLoGraF data.npz")
    parser.add_argument("--output-root", type=Path, default=Path("outputs/telograf/toy_squares/runs"))
...
class ToySquaresFlowDataset(Dataset):
...
def evaluate_ground_truth(records: List[Mapping]) -> Dict:
    ...
    for record in records:
        traj = np.asarray(record["trajs"], dtype=np.float32)
        ok, score = evaluate_spec_sequence(record["spec"], traj)
        total_ok += int(ok)
        scores.append(float(score))
        pads.append(float(record.get("padded_steps", 0)))
    ...
    return {
        ...
        "mean_padded_steps": float(np.mean(pads)) if pads else 0.0,
...
def evaluate_generated(
    ...
    for i in range(n):
        ...
        ok, score = evaluate_spec_sequence(record["spec"], states)
        ok_count += int(ok)
        scores.append(float(score))
...
def plot_xy_samples(path: Path, records: List[Mapping], title: str, max_items: int = 24) -> None:
    colors = {"blue": "#1e88ff", "red": "#ff4136", "green": "#34c759", "yellow": "#ffd60a"}
    fig, ax = plt.subplots(figsize=(5, 5))
    for record in records[:max_items]:
        traj = np.asarray(record["trajs"], dtype=np.float32)
        ax.plot(traj[:, 0], traj[:, 1], alpha=0.35, linewidth=1.0)
        blocks = traj[0, 2:10].reshape(4, 2)
...
def main() -> None:
    ...
    run_name = args.name or f"toy_squares_flow_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    train_dataset = ToySquaresFlowDataset(train_records, stats)
    valid_dataset = ToySquaresFlowDataset(valid_records, stats)
    ...
    if not args.skip_sampling:
        diffuser.eval()
        gen_eval = evaluate_generated(diffuser, valid_dataset, valid_records, stats, device, max_samples=args.eval_samples)

    torch.save(make_checkpoint_payload(), run_dir / "checkpoint.pt")
    np.savez(run_dir / "normalization_stats.npz", **{k: v for k, v in stats.items()})
    write_json(
